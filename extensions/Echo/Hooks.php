...
class EchoHooks {
	 ...
	 * @param $echoType Mixed: The methods specified term
	 * @param $operation Mixed: user name
	 * @param $belUserId Mixed: The message to send to user id
	 * @param $type Mixed:
	 * 					  type     true      false
	 * 		  article-thumb-up    comment    content
	 * 		  article-comments    comment    reply
	 *        article-cite-my
	 * @return Integer:
	 */
	public static function onAddMyNotification($echoType,$belsuerid,$article,$synopsis,$actionid,$from,$wikikey,$type,$other=null){
		...
		if(in_array($echoType,$array)){
			...
			if(!$type || !$belsuerid || !$article || !$synopsis || !$from || !$actionid || !$wikikey){
				wfDebugLog( __CLASS__, __FUNCTION__ . ": Parameter is null for $echoType" );
			}else{
				EchoEvent::createNewType( array(
					'type' => $echoType,
					'agent' => $wgUser,
					'extra' => array(
	...
	//关注我的
	public static function onAddNewConsiderMe( $be_user,$userName,$userid,$type=true){

		global $wgUser;
		EchoEvent::createNewType( array(
			'type' => 'article-consider-me',
			'agent' => $wgUser,
			'extra' => array(
