...
class EchoEvent extends EchoAbstractEntity{
	...
	public static function createNewType( $info = array() ,$be_user) {
		global $wgEchoNotifications;
		...
		if( $be_user){
			...
			if(count($permissions)){
				if(in_array('echo-subscriptions-web-'.$info['type'],$permissions)){
					return true;
				}
			}
		}
		...
		EchoNotificationController::newNotify( $obj, $wgEchoUseJobQueue  );
	...
	public function toDbArray() {
		...
		if(EchoEvent::$be_user_id){
			$data['event_agent_id'] = EchoEvent::$be_user_id;
		}
		if ( $this->pageId ) {
