...
class EchoNotificationController {
	...
	public static function newNotify( $event, $defer = true ) {
		...
		$userNotifyTypes = self::getEventNotifyTypes( $type );

		foreach ( $userNotifyTypes as $type ) {
			self::doNotification( $event, $wgUser, $type );
		}
		$userIds = array();
		$userIdsCount = 0;
		Hooks::run( 'EchoGetNotificationTypes', array( $wgUser, $event, &$userNotifyTypes ) );
