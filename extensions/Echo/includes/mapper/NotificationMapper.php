...
class EchoNotificationMapper extends EchoAbstractMapper {
	...
	public function insert( EchoNotification $notification ) {
		...
		if(EchoEvent::$be_user_id){
			$row['notification_user'] = EchoEvent::$be_user_id;
		}
	 ...
	 * @return EchoNotification[]
	 */
	public function fetchByUser( User $user, $limit, $continue, array $eventTypes = array(), array $excludeEventIds = array() ,$skip = false , $event_type = false) {
		...
		if( $skip ){
			...
			$fields = 'event_type,count(*) as num';
			$where = array(
				'GROUP BY' => 'event_type'
			);
		}else{
