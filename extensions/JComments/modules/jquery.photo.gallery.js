...
$.extend({
	//打开图片查看器
	openPhotoGallery : function(obj,elParent,elSon){
		...
		var $gallerys = $(obj).closest("."+oPar),
			activeIndex=0,
			imgs = [];
		$gallerys.find("."+oSon).each(function(i, elem){
			var url = this.src,
				img = $(this)[0],
		...
		console.log(localStorage["photoGalleryImgs"]);
		
		$("#J_pg").remove();
		$("<iframe></iframe").appendTo("body")
			.attr("id", "J_pg")
			.attr("src", window.location.pathname.match(/\/(.+)\//)[1] + "/extensions/JComments/modules/gallery.html?" + Math.random())
			.css({
				position : "fixed",
