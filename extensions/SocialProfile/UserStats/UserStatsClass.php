...
class UserStatsTrack {
	...
	function clearCache() {
		...
		$key = wfForeignMemcKey('homewiki', false, 'user', 'stats', $this->user_id );
		$wgMemc->delete( $key );
	}
	...
	function incStatField( $field, $val = 1 ) {
		...
		if ( !$wgUser->isAllowed( 'bot' ) && $this->stats_fields[$field] ) {
			$dbw = wfGetDB( DB_MASTER );
			$dbw->update(
				'user_stats',
	 ...
	 */
	function clearBoardCount($type=1) {
		global $wgUser;
	
		if ( !$wgUser->isAnon() ) {
			$dbw = wfGetDB( DB_MASTER );
			$field = $type==1?'user_board_count':'user_board_count_priv';
			$res = $dbw->update(
					'user_stats',
					array(
							$field => '0'
...
class UserStats {
	...
	public function getUserStatsDB() {
		...
		$uarow = $dbr->fetchObject( $uares );
		
		$stats['profileid'] = isset( $uarow->profileid ) ? $uarow->profileid : '';
		$stats['total_like_count'] = isset( $uarow->total_like_count ) ? $uarow->total_like_count : 0;
		$stats['total_comment_count'] = isset( $uarow->total_comment_count ) ? $uarow->total_comment_count : 0;
		$stats['total_edit_count'] = isset( $uarow->total_edit_count ) ? $uarow->total_edit_count : 0;
		$stats['brief'] = isset( $uarow->brief ) ? $uarow->brief : '';
		$stats['interest'] = isset( $uarow->interest ) ? $uarow->interest : '';
		$stats['is_attention'] = isset( $uarow->is_attention ) ? $uarow->is_attention : '';
		$stats['is_secretchat'] = isset( $uarow->is_secretchat ) ? $uarow->is_secretchat : '';
