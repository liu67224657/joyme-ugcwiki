...
class Vote {
	...
	function getAverageVote() {
		...
		return number_format( $voteAvg, 1 );
	}
...
class VoteStars extends Vote {
	...
	function display( $voted = false ) {
		...
		$rating_score_bg = 'rating-score-bg'.intval($display_stars_rating);

	 	$id = $this->PageID;

		// Should probably be $this->PageID or something?
		// 'cause we define $id just above as an empty string...duh
		$output = '<div id="rating_' . $id . '" class="vote-main">';
		$output .= '<div class="rating-score '.$rating_score_bg.'">';
		...
		$count = $this->count()>9999?'9999+':$this->count();

		//投票总数
		$output .= '<br/><span class="rating-total">' . wfMessage( 'voteny-votes', $count )->parse() . '</span>';
		...
		if ( $already_voted ) {
			$output .= '<div><span class="rating-voted">' .
	 ...
	 * @return string Generated <img> tag
	 */
	function displayGetStarsRs( $id, $rating, $voted ) {
		global $wgExtensionAssetsPath;
		...
		for ( $x = 1; $x <= $this->maxRating; $x++ ) {
			...
			$output .= "<img src=\"{$wgExtensionAssetsPath}/VoteNY/images/star_";
			switch ( true ) {
				case $rating >= $x:
					if ( $voted ) {
						$output .= 'voted';
