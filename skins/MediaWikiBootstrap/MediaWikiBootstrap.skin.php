...
class SkinMediaWikiBootstrap extends SkinTemplate {
   ...
   function setupSkinUserCss( OutputPage $out ) {
        ...
        $styles = array( 'mediawiki.skinning.interface', 'skins.mediawikibootstrap','ext.socialprofile.userinfo.css' );
...
class MediaWikiBootstrapTemplate extends BaseTemplate {
    ...
    public function execute() {
                ...
                <div id="bodyContent" class="row">
                    ...
                    <div id="innerbodycontent" class="setting-con">
						...
						<div class="wiki-person">
			                        <h2 id="joyme_site_follow_status">
			                        	<b><a href="/<?=$wgWikiname?>/"><?=$wgSiteGameTitle;?></a></b>
			                        	<?php
			                        		if($wgIsLogin){
			                        			$ret = JoymeWikiUser::getUserFollowSite($wgUser->getId(),$wgSiteId);
			                        			if($ret && $ret->status==1){
			                        				echo '<a id="joymesiteuserfollow" href="javascript:;" class="sf-gz gz-done" onclick="mw.joymesiteuserfollow_ygz()"></a>';
			                        			}elseif($ret && ($ret->status==2 || $ret->status==3)){
			                        				echo '<a id="joymesiteuserfollow" href="javascript:;" class="sf-gz gz-done" onclick="mw.joymesiteuserfollow()"></a>';
			                        			}else{
			                        				echo '<a id="joymesiteuserfollow" href="javascript:;" class="sf-gz usersitefollow" onclick="mw.joymesiteuserfollow()"></a>';
			                        			}
			                        		}else{
			                        			echo '<a id="joymesiteuserfollow" href="javascript:;" class="sf-gz usersitefollow" onclick="mw.joymesiteuserfollow()"></a>';
			                        		}
			                        ...
			                        ?>
			                        <p class="focus-num"><em id="joyme_wiki_word_info"><?=$site_page_count?></em>词条已创建 ,<em id="joyme_wiki_fillow_info"><?=$site_follow_usercount?></em>用户已关注</p>
			                    </div>
                                <?php $area1 = false; $areacontent = SpecialRecommendArea::getAreaContent( '推荐区1' ); if($this->data['isarticle'] && $this->data['articleid']!=1 && !empty($areacontent)){$area1=true;}
                                   if($area1){
                                      echo '<div class="nav-left-tj fn-clear">
                                            <div class="left-icon">
                                                <span class="tj-bg"></span>
                                                </div>
                                                <div class="left-tj-con">'.
                                                $areacontent
                                                .'</div>
                                            </div>';
                                   }
                                ;?>
			                    <div class="wiki-search web-hide">
                        		...
                        		<div class="nry-section1">
									<div id="firstHeading2" class="firstHeading2 page-header nry-h1 fn-clear">
                                        <?php if($this->data['isarticle']):?>
											...
											<?php
											 if($this->data['articleid'] && $this->data['isarticle'] && $wgTitle->mNamespace == 0): 
                                            ...
                                            <?php endif;?>
                                            <div class="select-choose web-hide">
                                        ...
                                        <?php else: ?>
                                            <div class="h1-top">
                                                <h1 class="h1" dir="auto"><?php $this->html('title') ?></h1>
                                                <div class="fn-clear"></div>
                                            </div>
                                        <?php endif;?>
							         </div>
							         <div class="nry-main-content">
								 ...
								 <div class="tjq-section">
								    <?php if($this->data['isarticle'] && $this->data['articleid']!=1){ echo SpecialRecommendArea::getAreaContent( '推荐区2' );}?>
                                </div>
        ...
        <?php if($wgIsLogin):?>
        <div class="gl-tool">
			<div class="gl-tool-con">
			    <ul>
			        <li class="gjt">工具条</li>
					<li class="create-newpage"><a href="javascript:;">创建新页面</a>
					<div class="create-wiki-wrap">
                        	...
                        	if (!empty($val['list'])){
                        		...
                        		foreach ($val['list'] as $b) {
                        			if (!empty($b['list'])){
                        				...
                        				echo '<li class="ej"><a class="'.$this->getSidebarOn($b['info']["href"]).'" href="' . $b['info']["href"] . '">' . $b['info']["text"] . '</a>';
                        			}
                        			echo "</li>";
