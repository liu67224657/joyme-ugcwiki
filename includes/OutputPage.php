...
class OutputPage extends ContextSource {
	...
	public function redirectHome( $url ){
		global $wgWikiname;
		if($wgWikiname != 'home'){
			$this->setPageTitle( $this->msg( 'ur-error-page-title' )->plain() );
			$this->redirect( '/home/index.php?title='.$url );
			return false;
		}
	}
	...
	public function getSeoInfo(){
		global $wgSiteGameTitle,$wgSiteSEOKeywords,$wgSiteSEODescription;
		
		$seoinfo = '';
		
		if ($this->getPageTitle() == '首页' || $this->getPageTitle() == '%E9%A6%96%E9%A1%B5' || urlencode('$this->getPageTitle()') == '%CA%D7%D2%B3' ||
				urlencode($this->getPageTitle()) == '%E9%A6%96%E9%A1%B5'
		) {
			$seoinfo .= Html::element( 'title', null, $this->getHTMLTitle() ) . "\n";
		
			$seoinfo .= Html::element( 'meta',
					array( 'name' => 'keywords', 'content' => $wgSiteSEOKeywords),
					null ) . "\n";
			
			$seoinfo .= Html::element( 'meta',
					array( 'name' => 'description', 'content' => $wgSiteSEODescription ),
					null ) . "\n";
		} else {
			$seoinfo .= Html::element( 'title', null, $this->getHTMLTitle() ) . "\n";
		
			$seoinfo .= Html::element( 'meta',
					array( 'name' => 'keywords', 'content' => $this->getPageTitle().','.$wgSiteGameTitle.' '.$this->getPageTitle()),
					null ) . "\n";
		}
	...
	public function getHTMLTitle() {
        ...
        if ($this->getPageTitle() == '首页' || $this->getPageTitle() == '%E9%A6%96%E9%A1%B5' || urlencode('$this->getPageTitle()') == '%CA%D7%D2%B3' ||
            ...
            return $this->mHTMLtitle . '_着迷网';
        } else {
            return $this->getHTMLTitleRewrite();
        }
    }
    
    public function getHTMLKeywords() {
    	global $wgSiteGameTitle;

        $HTMLTitle = $this->mHTMLtitle;
        $HTMLTitle_arry = explode('_', $HTMLTitle);
        $mTitle = '_';
        if (!empty($wgSiteGameTitle)) {
            $mTitle = $this->getPageTitle() . ',' . $wgSiteGameTitle;
        } elseif (count($HTMLTitle_arry) > 1) {
            $mTitle = $this->getPageTitle() . ',' . $HTMLTitle_arry[(count($HTMLTitle_arry) - 1)];
        } else {
            $mTitle = $HTMLTitle;
        }
        return $mTitle . '_着迷网';
	...
	//在此方法下面增加如下代码
    public function getHTMLTitleRewrite() {
        global $wgSiteGameTitle;
        ...
        if (!empty($wgSiteGameTitle)) {
            $mTitle = $HTMLTitle . '_' . $wgSiteGameTitle;
        } elseif (count($HTMLTitle_arry) > 1) {
            $mTitle = $HTMLTitle . '_' . $HTMLTitle_arry[(count($HTMLTitle_arry) - 1)];
        } else {
            $mTitle = $HTMLTitle;
        }
        return $mTitle . '_着迷网';
	...
	public function headElement( Skin $sk, $includeStyle = true ) {
		global $wgContLang, $wgMimeType,$wgSiteSEOKeywords,$wgSiteSEODescription, $wgServer,$wgUseWikiSeo;
		...
		//ppadd start
		global $wgDefaultDevice,$wgNoflow;
		$ret .= Html::element('meta', array('content' => 'no-transform','http-equiv'=>'Cache-Control')) . "\n";
		$ret .= Html::element('meta', array('content' => 'width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0','name'=>'viewport')) . "\n";
		$ret .= Html::element('meta', array('http-equiv' => 'X-UA-Compatible', 'content' => 'IE=edge,Chrome=1')) . "\n";
		//$ret .= Html::element('meta', array('content' => 'yes','name'=>'apple-mobile-web-app-capable')) . "\n";
		//$ret .= Html::element('meta', array('content' => 'black','name'=>'apple-mobile-web-app-status-bar-style')) . "\n";
		//$ret .= Html::element('meta', array('content' => 'telephone=no','name'=>'format-detection')) . "\n";
		

		if(!empty($wgNoflow)){
			$ret .= Html::element('meta', array('name' => 'robots', 'content' => 'nofollow')) . "\n";
		}
	...
	function getInlineHeadScripts() {
		...
		$tmpdata = $this->getJSVars();
		global $wgWikiname,$wgLoadphpCache,$wgServer,$wgEnv,$wgPhpServer,$wgJoymeUserInfo;
		if($wgLoadphpCache){
			$wgLoadphpCacheStr = "true";
		}else{
			$wgLoadphpCacheStr = "false";
		}
		if($wgJoymeUserInfo){
			$wgUserProfileId = $wgJoymeUserInfo['profileid'];
		}else{
			$wgUserProfileId = '';
		}
		$links[] = Html::inlineScript('window.wgWikiCom="'.$wgEnv.'";window.wgPhpServer ="'.$wgPhpServer.'";window.wgLoadphpCache ='.$wgLoadphpCacheStr.';window.wgWikiname = "'.$wgWikiname.'";window.articleId = '.$tmpdata['wgArticleId'].';window.wgServer ="'.$wgServer.'";window.wgUserProfileId="'.$wgUserProfileId.'";');
