...
class User implements IDBAccessObject {
	...
	public function load( $flags = self::READ_NORMAL ) {
		global $wgFullyInitialised,$joyme_u_status;
		...
		switch ( $this->mFrom ) {
			...
			case 'id':
				...
				break;
			case 'session':
				if(!empty($joyme_u_status) && $joyme_u_status == 'on'){
					if ( !$this->loadFromJoyme() ) {
						// Loading from session failed. Load defaults.
						$this->loadDefaults();
					}
				}elseif ( !$this->loadFromSession() ) {
					// Loading from session failed. Load defaults.
					$this->loadDefaults();
				}
	 ...
	 */
	private function loadFromJoyme() {
		global $joyme_u_key;
		JoymeUser::initByRequest();
		$sId = JoymeUser::getUid();
		if(empty($sId)){
			return false;
		}
	...
	public function loadFromDatabase( $flags = self::READ_LATEST ) {
		...
		if ( $s !== false ) {
			$this->jmuc_checknick( $s );
			return false;
		}elseif ($this->jmuc_register($this->mId)){
	...
	public function jmuc_register($uid){
		JoymeUser::initByRequest();
		$current_uid = JoymeUser::getUid();
		if($current_uid == $uid){
			...
			$res = $joymewikiuser->getProfilebyid($profileid);
			if(empty($res)){
				return false;
			}
		}else{
			$joymewikiuser = new joymeWikiUser();
			$res = $joymewikiuser->getProfilebyUid($uid);
			if(empty($res)){
				return false;
			}
		}
		...
		return $this->jmuc_reguser($res[0]['uid'],$res[0]['nick'],$res[0]['profileid']);
	}

	public function jmuc_reguser($uid,$nick,$profileid){
		...
		$nick = ucfirst(str_replace('+', ' ', $nick));
		$nick = ucfirst(str_replace('~', ' ', $nick));
//		$nick = ucfirst(preg_replace("/\s/","",$nick));

		$dbr = wfGetDB(DB_SLAVE);
		$user_count = $dbr->selectRowCount(
			'user',
			'*',
			array('user_id' => $uid),
			__METHOD__
		...
		if(empty($user_count)){
			$newUser = $this->createNew($nick,array('id'=>$uid,'password'=>':B:cc284d78:5cdade62046fb7d1d956944ce6b941f3','token'=>'172f1b14d720510f55fa58ec15f59a6f'));
			if($newUser){
				$dbw = wfGetDB( DB_MASTER );
				if(in_array($uid, $joyme_u_adminid)){
					$data = array(
						'ug_user' => $uid,
						'ug_group' => 'bureaucrat',
					);
					$dbw->insert( 'user_groups', $data, __METHOD__ );

					$joymewikiuser = new JoymeWikiUser();
					$joymewikiuser->addUserSiteManage($uid);
				}
				...
				if (empty($user_addition_count)) {
					...
					if(empty($ret)){
						Log::error('profileid for '.$uid.' failed insert into user_addition.' );
					}
				}else{
					$ret = $dbw->update(
						'user_addition',
					...
					if(empty($ret)){
						Log::error('profileid for '.$uid.' failed update user_addition.' );
					}
				}
//				$key = wfForeignMemcKey('homewiki',false,'user','id',$uid);
				$key = wfMemcKey( 'user', 'id', $uid );
	...
	public function jmuc_checknick($user){
		global $wgJoymeUserInfo,$wgJoymeUserInfoCheck;
		if($user && $wgJoymeUserInfoCheck==false){
			$this->jmuc_joymeuserinfo();
			if($wgJoymeUserInfo){
				if($wgJoymeUserInfo['uid'] == $user->user_id && !empty($wgJoymeUserInfo['nick'])){
					if(ucfirst($wgJoymeUserInfo['nick']) != $user->user_name){
						$dbw = wfGetDB( DB_MASTER );
						$dbw->update(
							'user',
							array(
								'user_name' => ucfirst($wgJoymeUserInfo['nick']),
							),
						...
						$user->user_name = ucfirst($wgJoymeUserInfo['nick']);

//					$key = wfForeignMemcKey('homewiki',false,'user','id',$user->user_id);
						$key = wfMemcKey('user','id',$user->user_id);
						$data = ObjectCache::getMainWANInstance()->get( $key );
						if($data){
							ObjectCache::getMainWANInstance()->delete( $key );
						}
				...
				$wgJoymeUserInfoCheck = true;
			}
		}
		// Initialise user table data
		$this->loadFromRow( $user );
		$this->mGroups = null; // deferred
		$this->getEditCount(); // revalidation for nulls
		return true;
	}

	public function jmuc_joymeuserinfo(){
		global $wgJoymeUserInfo;

		if($wgJoymeUserInfo){
			$joymewikiuser = new JoymeWikiUser();
			$joymewikiuser->getProfilebyid($wgJoymeUserInfo['profileid']);
			$joymewikiuser->getUserAddition($wgJoymeUserInfo['uid']);

			$wgJoymeUserInfo['mobile'] = $joymewikiuser->mobile;
			$wgJoymeUserInfo['nick'] = $joymewikiuser->nick;
			$wgJoymeUserInfo['sex'] = $joymewikiuser->sex;
			$wgJoymeUserInfo['proviceid'] = $joymewikiuser->proviceid;
			$wgJoymeUserInfo['interest'] = $joymewikiuser->interest;
			$wgJoymeUserInfo['icon'] = $joymewikiuser->icon;
			$wgJoymeUserInfo['brief'] = $joymewikiuser->brief;
			$wgJoymeUserInfo['birthday'] = $joymewikiuser->birthday;
			$wgJoymeUserInfo['is_attention'] = $joymewikiuser->is_attention;
			$wgJoymeUserInfo['is_secretchat'] = $joymewikiuser->is_secretchat;
			$wgJoymeUserInfo['bindsinaflag'] = $joymewikiuser->bindsinaflag;
			$wgJoymeUserInfo['bindqqflag'] = $joymewikiuser->bindqqflag;
			$wgJoymeUserInfo['bindmobileflag'] = $joymewikiuser->bindmobileflag;

			if(empty($joymewikiuser->user_profileid)){
				$dbw = wfGetDB(DB_MASTER);
				$result = $dbw->update(
					'user_addition',
					array(
						'profileid' => $wgJoymeUserInfo['profileid']
					),
					array(
				...
				$dbw->commit(__METHOD__);
				if(empty($result)){
					Log::error('profileid for '.$wgJoymeUserInfo['uid'].' is empty in user_addition.' );
				}
			}

			if(!empty($wgJoymeUserInfo['nick'])){
				if(!preg_match('/^[0-9a-zA-Z\x{4e00}-\x{9fa5}]+$/u',$wgJoymeUserInfo['nick'])){
					global $wgEnv,$wgLoginDomain;
					$url = 'http://passport.joyme.'.$wgEnv.'/auth/savenickpage?reurl=http://wiki.joyme.'.$wgEnv.$_SERVER['REQUEST_URI'].'&logindomain='.$wgLoginDomain;
