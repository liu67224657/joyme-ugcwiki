...
abstract class File implements IDBAccessObject {
	...
	public function getUrl() {
		...
		if(!empty($this->aurl)){
			$aurl = $this->aurl;
			$info = $this->getImgInfo();
			$this->url = Joyme::imgFormQiNiu($aurl,$info);
		}elseif ( !isset( $this->url ) ) {
			$this->assertRepoDefined();
			$ext = $this->getExtension();
			$info = $this->getImgInfo();
			$this->url = Joyme::imgFormQiNiu('/wiki'.$this->repo->getZoneUrl( 'public', $ext ) . '/' . $this->getUrlRel(),$info);
		}
		return $this->url;
	}
	...
	public function getImgInfo(){
		global $wgWikiname,$IP;

		//$img_sha1 = $this->getSha1();
		$img_name = $this->getName();
		$path = $IP.'/cache/'.$wgWikiname.'/imgname';
