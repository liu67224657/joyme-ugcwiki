	...
	function upload( $src, $comment, $pageText, $flags = 0, $props = false,
		...
		if ( !$props ) {
			if ( $this->repo->isVirtualUrl( $srcPath )
				...
				$arr = explode("/",urldecode($srcPath));

				$name = explode("!",$arr[count($arr)-1]);//

				$dbr = wfGetDB( DB_MASTER );
				$conds = array( 'img_name' => $name[1] );
				$props = $dbr->selectRow( 'image', array('img_width','img_height','img_bits','img_minor_mime','img_major_mime','img_sha1','img_major_mime','img_size','img_metadata','img_media_type'), $conds, __METHOD__ );

				$result = array();
				$object = is_object($props) ? get_object_vars($props) : $props;
				foreach ($object as $key => $val) {
					$val = (is_object($val) || is_array($val)) ? objectToArray($val) : $val;
					$result[$key] = $val;
				}
				$result['width'] = $result['img_width'];
				$result['height'] = $result['img_height'];
				$result['bits'] = $result['img_bits'];
				$result['fileExists'] = 1;
				$result['mime'] = $result['img_major_mime'];
				$result['media_type'] = $result['img_media_type'];
				$result['metadata'] = $result['img_metadata'];
				$result['sha1'] = $result['img_sha1'];
				$result['major_mime'] = $result['img_major_mime'];
				$result['minor_mime'] = $result['img_minor_mime'];
				$result['size'] = $result['img_size'];
				$props = $result;
			} else {
