...
class SpecialBlock extends FormSpecialPage {
	...
	public static function processForm( array $data, IContextSource $context ) {
		...
		//var_dump(intval($data['point']),$data['point']);exit;
		if ( !empty($data['point']) && (intval($data['point'])<=0 || $data['point'] != intval($data['point']) )) {
			return [ '扣除积分请输入正整数' ];
		}
		...
		//扣除积分
		if ( intval($data['point'])>0 && !empty($userId)) {
			$joymewikiuser = new JoymeWikiUser();
	        //用户profileid
	        $profileid = $joymewikiuser->getProfileid($userId);
	        if (!empty($profileid)) {
	            $rs = $joymewikiuser::reducepoint($profileid, intval($data['point']));
	        }
		}
