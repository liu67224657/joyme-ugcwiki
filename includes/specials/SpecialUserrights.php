	...
	public function execute( $par ) {
		...
		if (
			...
			if ( $request->getVal( 'conflictcheck-originalgroups' )
				...
				$wpGroupsysop = $request->getVal( 'wpGroup-sysop' , 0);
				$wpGroupbureaucrat = $request->getVal( 'wpGroup-bureaucrat' , 0 );
				//分配管理员
				if((isset($wpGroupsysop)
					&&$wpGroupsysop==1)
					||(isset($wpGroupbureaucrat)
					&&$wpGroupbureaucrat==1)
				){
					JoymeWikiUser::addUserSiteManage($targetUser->mId);
				}
				//取消管理员
				elseif ($wpGroupsysop == 0 && $wpGroupbureaucrat==0){
					JoymeWikiUser::delUserSiteManage($targetUser->mId);
				}
