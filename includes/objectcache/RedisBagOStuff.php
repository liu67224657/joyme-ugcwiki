...
class RedisBagOStuff extends BagOStuff {
	...
	public function keys( $key ) {
		...
		$this->logRequest( 'keys', $key, $server, $result );
		return $result;
	}
	public function zIncrBy( $key ,$val,$member) {
		list( $server, $conn ) = $this->getConnection( $key );
		if ( !$conn ) {
			return false;
		}
		try {
			$conn->zIncrBy( $key,$val,$member );
			// Return true even if the key didn't exist
			$result = true;
		} catch ( RedisException $e ) {
			$result = false;
		...
		$this->logRequest( 'zIncrBy', $key, $server, $result );
		return $result;
	}
	public function zRange( $key,$limit=10 ) {
		list( $server, $conn ) = $this->getConnection( $key );
		if ( !$conn ) {
			return false;
		}
		try {
			$result = $conn->zRange( $key ,-$limit,-1,true);
			if(!empty($result)){
				arsort($result);
			}
		} catch ( RedisException $e ) {
		...
		$this->logRequest( 'zRange', $key, $server, $result );
		return $result;
	}
	public function zScore( $key ,$member) {
		...
		$this->logRequest( 'zScore', $key, $server, $result );
		return $result;
	}
