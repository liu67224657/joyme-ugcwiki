...
abstract class Skin extends ContextSource {
	...
	function addToSidebarPlain( &$bar, $text ) {
		...
		foreach ($lines as $line) {
			...
			$msgLink = $this->msg( $href )->inContentLanguage();
			if ( $msgLink->exists() ) {
				$href = $msgLink->text();
				if ( $href == '-' ) {
					continue;
			...
			$msgText = $this->msg( $text );
			if ( $msgText->exists() ) {
				$text = $msgText->text();
			}
			
			if ((substr($href, 0, 7) !== 'http://') && ($href !== '#')) {
				$href = $wgServer. '/' .$wgWikiname.'/'. $href;
			}
			$bar_arr = array(
					'text' => $text,
					'href' => $href,
			...
			if (strpos($line, '***') === 0) {
				$heading3 = $text;
				$bar[$heading]['list'][$heading2]['list'][$heading3]['info'] = $bar_arr;
				$bar[$heading]['list'][$heading2]['list'][$heading3]['list'] = array();
			} elseif (strpos($line, '**') === 0) {
				$heading2 = $text;
				$bar[$heading]['list'][$heading2]['info'] = $bar_arr;
				$bar[$heading]['list'][$heading2]['list'] = array();
			} else {
				$heading = $text;
				$bar[$heading]['info'] = $bar_arr;
				$bar[$heading]['list'] = array();
			}
		}
		return $bar;
	}
