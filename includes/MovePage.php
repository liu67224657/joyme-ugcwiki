...
class MovePage {
	...
	public function move( User $user, $reason, $createRedirect ) {
		...
		$strSubModel = new DataSynchronization();
		...
		foreach ( $prefixes as $prefixRow ) {
			...
			$col_name = Collation::singleton()->getSortKey($this->newTitle->getCategorySortkey( $prefix ));
			for($i=0;$i<=strlen($col_name);$i++){
				$result_name = $strSubModel->msubstr($col_name,$i,1);
				$result = $PingYing->Pinyin($result_name,'utf-8');
				$first = strtoupper(substr(trim($result),0,1));
				if(preg_match("/[0-9A-Za-z]/",$first)){
					$result = $first;
					break;
				}
			}
